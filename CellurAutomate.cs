...
// p.s. - комметарии в этом коде используются для перевода между языком мат.модели и программистом, т.е. чтобы не запуститься.
// если использовать понятные людям названия - тогда можно запутаться при программирование самой модели и наоборот. Надеюсь вы меня поняли.
